...
| `geometric_mean_prob` | `exp(mean_logprob)` | `mt.lib_mt_confidence.geometric_mean_prob` | Same caveats as `mean_logprob`; values saturate near high confidence | Compare with Park et al. (2025) geometric token confidence |
...
| `top-k kurtosis` | `E[(p−mean)^4] / variance^2` over returned top-k probs | `mt.lib_mt_confidence.token_topk_kurtosis` | Only as good as returned top-k; OpenAI-compatible servers often return far less than top-1000 | Token-distribution peakedness from uncertainty visualisation work |
...
| `mean_prob_all` | Mean `prob` over all scored tokens | `mt.lib_mt_confidence.content_token_summary` | Includes punctuation and function words | Compare with Park et al. (2025) arithmetic token confidence |
...
| `mean_topk_kurtosis` | Mean top-k kurtosis over content tokens | `mt.lib_mt_confidence.content_token_summary` | Not comparable when servers return different top-k widths | Validate vs BLEU / chrF / MQM before treating as confidence |
