#!/usr/bin/env python3
"""
Demo: compare a good vs bad German translation of an English sentence.

Reads the good-hypothesis logprobs from QElogprob.json (saved response),
fetches the bad-hypothesis logprobs live from the API, then prints a
side-by-side score comparison.

Usage:
    python3 example_qe.py --base-url http://localhost:8000/v1/chat/completions \
        --model <your-model-id>

Use --strict-rank1 for naive rank==1 parsing (often mis-aligns on some models).
"""
...
def rank1_strict_extract(prompt_logprobs, marker: str) -> list[tuple[str, float]]:
    """For each position after marker, take rank == 1 → logprob."""
...
def main() -> int:
    ...
    p.add_argument("--strict-rank1", action="store_true", help="Use naive rank==1 extraction")
    p.add_argument("--marker", default=MARKER, help=f'Marker string (default: "{MARKER}")')
    p.add_argument("--base-url", default=DEFAULT_BASE_URL, help="Chat completions endpoint")
    p.add_argument("--model", default=DEFAULT_MODEL, help="Model id (required for live call)")
    args = p.parse_args()
